...
exports.cardsRouter.post('/authorize', async (req, res) => {
    try {
        ...
        if (cardDoc && cardDoc.status === 'blocked')
            return res.status(403).json({ error: 'Card blocked' });
        // Fraud decision via gRPC
        const decision = await (0, grpc_1.assessCard)(amt, merchant, cardType, last4);
        if (!decision.approved) {
            // Upsert masked card metadata (no CVV persisted)
            await cards.updateOne({ last4, cardType }, { $set: { maskedNumber: masked, cardType, expiry, status: 'declined', updatedAt: new Date().toISOString() } }, { upsert: true });
        ...
        await cards.updateOne({ last4, cardType }, { $set: { maskedNumber: masked, cardType, expiry, status: 'active', updatedAt: new Date().toISOString() } }, { upsert: true });
        // Ensure producer
        if (!producerReady) {
