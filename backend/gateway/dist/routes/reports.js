...
exports.reportsRouter.get('/complaints.csv', async (_req, res) => {
    try {
        const { rows } = await pool.query(`SELECT to_char(le.at, 'YYYY-MM-DD"T"HH24:MI:SSZ') as at,
              le.tx_id,
              pm.complaints
...
exports.reportsRouter.get('/accounts.json', async (_req, res) => {
    try {
...
exports.reportsRouter.get('/reconciliation.json', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT to_char(day, 'YYYY-MM-DD') AS day, credits, debits, closing_balance, difference, status
...
exports.reportsRouter.get('/monthly.csv', async (_req, res) => {
    try {
        const { rows } = await pool.query(`SELECT to_char(date_trunc('month', le.at), 'YYYY-MM') AS month,
...
exports.reportsRouter.get('/transactions.csv', async (_req, res) => {
    try {
        const { rows } = await pool.query(`SELECT to_char(at, 'YYYY-MM-DD"T"HH24:MI:SSZ') as at, tx_id, account_id, currency, amount, direction
       FROM ledger_entries
       ORDER BY at DESC
       LIMIT 1000`);
...
exports.reportsRouter.post('/reconciliation/run', async (req, res) => {
    try {
    ...
    `);
        ...
        for (const r of daily) {
            ...
            await pool.query(`INSERT INTO reconciliation_results(day, credits, debits, closing_balance, difference, status)
         VALUES ($1,$2,$3,$4,$5,$6)
         ON CONFLICT (day) DO UPDATE SET
           ...
           status = EXCLUDED.status`, [r.day, credits, debits, closing, difference, status]);
        }
...
exports.reportsRouter.get('/reconciliation.csv', async (_req, res) => {
    try {
        const { rows } = await pool.query(`SELECT to_char(day, 'YYYY-MM-DD') AS day, credits, debits, closing_balance, difference, status
       FROM reconciliation_results
       ORDER BY day DESC
