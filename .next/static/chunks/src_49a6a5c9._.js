...
function setToken(token) {
    ...
    try {
        var _payload_realm_access;
        const payload = JSON.parse(atob(token.split(".")[1] || ""));
        const roles = (payload === null || payload === void 0 ? void 0 : (_payload_realm_access = payload.realm_access) === null || _payload_realm_access === void 0 ? void 0 : _payload_realm_access.roles) || [];
...
function beginLogin() {
    ...
    const redirectUri = encodeURIComponent("".concat(window.location.origin, "/login/callback"));
    const authUrl = "http://localhost:8443/realms/atlas/protocol/openid-connect/auth?client_id=atlas-gateway&response_type=code&redirect_uri=".concat(redirectUri);
...
function registerLocalUser(username, password, email) {
    ...
    const roles = role === 'user' || role === 'customer' ? [
        'user'
    ] : [
        'user'
    ];
    ...
    saveUsers(users);
    if ("TURBOPACK compile-time truthy", 1) localStorage.setItem(CURRENT_USER_KEY, username);
    return true;
}
function localLogin(username, password, desiredRole) {
    // Built-in admin
...
function issueDummyToken(roles) {
    ...
    const token = "".concat(header, ".").concat(payload, ".");
    setToken(token);
}
...
function isAllowed(path) {
    ...
    const startsIn = (set)=>Array.from(set).some((k)=>p === k || p.startsWith(k + '/'));
...
_s(AuthGate, "qIbXzL/glMgPmW/TFWAi/FqCkIk=", false, function() {
    return [
...
//# sourceMappingURL=src_49a6a5c9._.js.map